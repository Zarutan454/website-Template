...
const FollowingList: React.FC<FollowingListProps> = ({ userId, limit = 5 }) => {
  ...
  // TODO: Diese Komponente muss auf Django-API migriert werden. useFollowSystem wurde entfernt.
  // const { getFollowing, isFollowing, followUser, unfollowUser } = useFollowSystem();
  
  useEffect(() => {
    const loadFollowing = async () => {
      setIsLoading(true);
      try {
        const userFollowing = await getFollowing(userId, limit);
        setFollowing(userFollowing);
        
        // Initialisieren der Follow-Status für alle Following
        if (currentUser?.id) {
          const statusMap: Record<string, boolean> = {};
          
          for (const user of userFollowing) {
  ...
  }, [userId, limit, getFollowing, currentUser?.id, isFollowing]);
  ...
  if (following.length === 0) {
    return (
      <div className="text-center py-4 text-gray-500">
        Folgt noch keinen anderen Benutzern.
      </div>
    );
  }
