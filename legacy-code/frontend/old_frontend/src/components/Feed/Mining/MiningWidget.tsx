...
const MiningWidget: React.FC<MiningWidgetProps> = ({ closePopover }) => {
  ...
  useEffect(() => {
    if (isMining && !isMiningHealthy && !syncAttempted) {
      console.log('Mining appears to be in an unhealthy state, attempting sync');
  ...
  useEffect(() => {
    const syncInterval = setInterval(() => {
      if (syncMiningState) {
        syncMiningState().then(status => {
          console.log("MiningWidget: Periodic sync completed");
        });
      }
  ...
  const handleMiningToggle = async () => {
    ...
    try {
      ...
      if (isMining) {
        console.log("Stopping mining from MiningWidget");
        ...
        console.log("Starting mining from MiningWidget");
  ...
  const mappedStats = {
    ...
    lastActivityAt: typeof miningStats.last_activity_at === 'string' 
      ...
      : (miningStats.last_activity_at ? new Date(miningStats.last_activity_at) : new Date())
  };
