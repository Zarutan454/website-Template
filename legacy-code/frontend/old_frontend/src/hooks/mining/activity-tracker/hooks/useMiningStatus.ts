...
export function useMiningStatus(
  ...
  useEffect(() => {
    const normalizedStats = normalizeMiningStats(initialStats);
    setTotalEarnings(normalizedStats.total_tokens_earned || 0);
  }, [initialStats.total_tokens_earned, initialStats.totalTokensEarned]);
  ...
  useEffect(() => {
    const calculateEfficiency = () => {
      ...
      if (normalizedStats.streak_days && normalizedStats.streak_days > 0) {
        baseEfficiency += Math.min(0.2, normalizedStats.streak_days * 0.05);
      }
  ...
  const onInteraction = useThrottledCallback(async (type: ActivityType, points: number, tokens: number) => {
    ...
    // Check if activity limit is reached - passing proper arguments
    if (isLimitReached(type, initialStats)) {
      return false;
    }
